...
def find_matched_name_file_num(source_file, test_file, source_name_str='patient_name', test_name_str='Patient Name',
    ...
    for test_index, test_clean_name in enumerate(test_clean_names):
        matched_name = process.extractOne(query=test_clean_name, choices=source_clean_names, scorer = fuzz.WRatio)
...
matched_names.to_excel(os.path.join(folder, '22_02_2021_names_file_number_score_WRatio.xlsx'))
